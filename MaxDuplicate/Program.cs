...
int[] MaxDuplicate(int numElements, int numSought, int maxElements){
    int count=0;
    int[] result= {0, numElements, numSought};
    int[] arr = new int[numElements];
    for (int i=0; i<numElements; i++){
        arr[i] = new Random().Next(0, maxElements+1);
    ...
    // нужна, если искомая последовательность замыкает массив
    if(result[0]<count){result[0]=count;}
    WriteLine();
...
WriteLine("Введите значение максимального элемента массива (от 1 до 9)");
int max = Convert.ToInt32(ReadLine());
WriteLine("Введите элемент для поиска");
int sought = Convert.ToInt32(ReadLine());
int[] maxDuplicate = MaxDuplicate(length, sought, max);
